...
function DiffView({ original, fixed, c }) {
  ...
  const isMobileView = window.innerWidth < 768;
  return (
    <div style={{ borderRadius: 10, overflow: 'hidden', border: `1px solid ${c.borderSoft}`, display: 'grid', gridTemplateColumns: isMobileView ? '1fr' : '1fr 1fr' }}>
      {['orig', 'fixed'].map(side => (
        <div key={side} style={{ borderRight: side === 'orig' ? `1px solid ${c.borderSoft}` : 'none' }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            {diff.map((row, i) => {
              ...
              return <div key={i} style={{ display: 'flex', gap: 8, padding: '0 10px', background: bg, minHeight: 22 }}>
                <span style={{ fontFamily: mono, fontSize: 11, color: c.text3, minWidth: 22, userSelect: 'none', lineHeight: '22px' }}>{i + 1}</span>
                <span style={{ fontFamily: mono, fontSize: 12, color: col, whiteSpace: 'pre', lineHeight: '22px', flex: 1 }}>{side === 'orig' ? row.orig : row.fixed}</span>
              </div>;
            })}
          </div>
        </div>
...
function LineNumberedEditor({ c, value, onChange, isDragging, highlightLine, tf }) {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0, position: 'relative' }}>
      {isDragging && <div style={{ position: 'absolute', inset: 0, background: c.isDark ? 'rgba(45,212,191,0.08)' : 'rgba(13,148,136,0.06)', border: `2px dashed ${c.teal}`, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 5 }}>
      ...
      <textarea ref={taRef} value={value} onChange={e => onChange(e.target.value)} onScroll={sync} spellCheck={false}
        style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', color: c.text1, fontFamily: mono, fontSize: 13, lineHeight: 1.8, padding: '1rem 1.25rem', resize: 'none', tabSize: 2, overflowY: 'auto', overflowX: 'auto', minWidth: 0, width: '100%' }} />
    </div>
  );
}
...
export default function App() {
  ...
  const placeholders = {
    javascript: '// Paste your JavaScript here...\nfunction example() {\n  let x = 10\n  console.log(x)\n}',
    nodejs: '// Paste your Node.js code here...\nconst express = require(\'express\')',
    python: '# Paste your Python code here...\ndef main():\n    pass',
    sql: '-- Paste your SQL query here...\nSELECT * FROM users',
    csharp: '// Paste your C# code here...\npublic void Example() { }',
    typescript: '// Paste your TypeScript here...\nconst greet = (name: string) => {}',
  ...
  useEffect(() => {
    ...
    s.id = 'fp-styles';
    s.textContent = `@keyframes fpFadeIn{from{opacity:0;transform:translateY(4px)}to{opacity:1;transform:translateY(0)}}@keyframes fpShimmer{0%,100%{opacity:.3}50%{opacity:.8}}*{-webkit-tap-highlight-color:transparent;}[style*="scrollbarWidth"]::-webkit-scrollbar{display:none;}`;
  ...
  const handleFixSingle = async (bug, idx) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error();
      const data = await res.json();
      if (data.fixedCode) { setCodeInput(data.fixedCode); showToast(t.copied); }
  ...
  return (
    <div style={{ minHeight: '100vh', background: c.bgBase, color: c.text1, fontFamily: tf, display: 'flex', flexDirection: 'column', transition: 'background 0.2s, color 0.2s', overflowX: 'hidden', width: '100%' }}>
      ...
      {/* ── Hero ── */}
      <div style={{ textAlign: 'center', padding: isMobile ? '1.5rem 1rem 0.5rem' : '2rem 1.25rem 0.75rem', position: 'relative', zIndex: 1 }}>
      ...
      {/* ── Main grid ── */}
      <main style={{ width: '100%', maxWidth: '100vw', overflowX: 'hidden', padding: isMobile ? '0.75rem 0.75rem 2rem' : '1.25rem 1.25rem 2rem', display: 'grid', gridTemplateColumns: isMobile || isTablet ? 'minmax(0,1fr)' : 'minmax(0,1fr) minmax(0,1fr)', gap: '1rem', flex: 1, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem', minWidth: 0, width: '100%' }}>
          <Panel c={c} style={{ flex: isMobile || isTablet ? 'none' : '1', minHeight: isMobile ? 280 : 380, height: isMobile || isTablet ? 320 : undefined, width: '100%', minWidth: 0 }}
            ...
            <LineNumberedEditor c={c} value={codeInput || placeholders[language] || ''} onChange={v => setCodeInput(v === placeholders[language] ? '' : v)} isDragging={isDragging} highlightLine={highlightLine} tf={tf} />
            {/* Line/char count */}
                ...
                {activeTab === 'fixed' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    {showDiff ? <DiffView original={originalCode} fixed={analysisResult.fixedCode} c={c} /> :
                      <SyntaxHighlighter language={langForHL} style={c.codeTheme} customStyle={{ margin: 0, borderRadius: 10, fontSize: 12.5, lineHeight: 1.75, background: c.codeBg }}>{analysisResult.fixedCode}</SyntaxHighlighter>}
                  </div>
                )}
                ...
                {activeTab === 'commented' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    {analysisResult.commentedCode
                      ? <SyntaxHighlighter language={langForHL} style={c.codeTheme} customStyle={{ margin: 0, borderRadius: 10, fontSize: 12.5, lineHeight: 1.75, background: c.codeBg }}>{analysisResult.commentedCode}</SyntaxHighlighter>
      ...
      {/* Status */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: mono, fontSize: 10, color: c.text3, padding: `0 ${isMobile ? '0.75rem' : '1.25rem'} 1rem`, flexWrap: 'wrap' }}>
